...
df_train_full = load_mel_features(df_train)
df_test_full = load_mel_features(df_test)

X_train = np.array(df_train_full['mel'].tolist())
y_train = np.array(df_train_full['Class ID'].tolist())
X_test = np.array(df_test_full['mel'].tolist())
y_test = np.array(df_test_full['Class ID'].tolist())

np.save('datasets/X_train_mel.npy', X_train)
np.save('datasets/y_train_mel.npy', y_train)
np.save('datasets/X_test_mel.npy', X_test)
np.save('datasets/y_test_mel.npy', y_test)
