...
model.summary()                                                                              #tf.keras.metrics.SparseCategoricalAccuracy()

X_train = np.load('datasets/X_train_mel.npy')
y_train = np.load('datasets/y_train_mel.npy')
X_test = np.load('datasets/X_test_mel.npy')
y_test = np.load('datasets/y_test_mel.npy')

batch_size = 32
num_epochs = 100

X_train, y_train = shuffle(X_train, y_train)  # This fixes an issue where validation accuracy was stuck at 0.33
...
model.save('model_mel_{}_epochs.h5'.format(num_epochs))
pd.DataFrame.from_dict(history.history).to_csv('history_mel_{}_epochs.csv'.format(num_epochs),index=False)
